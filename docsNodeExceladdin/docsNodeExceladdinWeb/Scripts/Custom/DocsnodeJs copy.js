...
BRDocsNodeJS.load = function () {
    ...
    var DocsNodeProductLogoListName = "DocsNodeProductLogo";
    var ConfigurationListName = "Configuration";
    var templateServerRelURL = "/sites/DocsNodeAdmin/" //Knut Env        
    var docsNodeXLSFileExtention = ".xls";
    var docsNodeXLSXFileExtention = ".xlsx";
    var docsNodeXLTFileExtention = ".xlt";
    var docsNodeXLTXFileExtention = ".xltx";
    var docsNodeListTemplateLogo = "xls-logo.png";
    ...
    this.init = function () {
        ...
        $(".SPTreeViewMore").click(function (e) {
    ...
    function getAllSites_Treeview_click(e) {
        if ($(e).siblings(".nested").length > 0) {
            ...
            var tenantDisplayName = SPURL.substr(8, SPURL.length);
            var selectedSiteURL = siteUrl.split("/sites/")[1];
            var cutmAttr = "";
            if (siteUrl.indexOf("/sites/") < 0 && tenantDisplayName != siteUrl.substr(8, SPURL.length)) {
                selectedSiteURL = siteUrl.substring(siteUrl.lastIndexOf("/") + 1, siteUrl.length);
                cutmAttr = "rootsubsites";
            }
            var rootSiteURL = siteUrl.split(tenantDisplayName)[1];
                    ...
                    getAllLibrayFromSite_Treeview(siteCollection, parentsiteKey, siteKey, customAttr, siteId).then(function (libData) {
                        if (libData && libData.length > 0) {
                            ...
                            for (var i = 0; i < listOfDocLibsArray.length; i++) {
                                liHTML += " <li id='" + parentsiteKey + "'><span class='caretCustom caret-down treeDocLib' ><div class='type' hidden>documentlibrary</div><div class='appWebUrl' hidden> " + listOfDocLibsArray[i].appWebUrl + "</div><div class='listName' hidden> " + listOfDocLibsArray[i].name + "</div><div class='selectedLibURL' hidden> " + listOfDocLibsArray[i].siteURL + "</div><div class='docLibKey' hidden> " + listOfDocLibsArray[i].docLibKey + "</div><a href='#'><i class='ms-Icon ms-Icon--FabricDocLibrary' aria-hidden='true'></i> " + listOfDocLibsArray[i].displayName + "</a></span>"; // main site collection shared doc li level - 2 open/close                    
    ...
    function createDocumentInDestLib_Treeview(checkNext) {
        try {
            ...
            if (docName == "" || docName == null || docName.match('%') || docName.match('"')
                ...
                var docnameLen = docName.length;
                var newFileName = docName + docsNodeXLSXFileExtention;
                            ...
                            if (flag == 1) {
                                ...
                                if (platform == "PC") {
                                    if (FolderRelativePath != null && FolderRelativePath != "") {
                                        getdocumentUrlsString = "<p>Your Document is saved <a class='docUrls' target='_blank' href='ms-excel:ofe|u|" + SPURL + FolderRelativePath + "/" + newFileName + "'> " + newFileName + ". </a></p>\n";
                                    }
                                    else {
                                        getdocumentUrlsString = "<p>Your Document is saved <a class='docUrls' target='_blank' href='ms-excel:ofe|u|" + SPURL + DocLibraryUrl + "/" + newFileName + "'> " + newFileName + ". </a></p>\n";
                                    }
                                ...
                                $("#DocumentUrls").css('display', 'block');
    ...
    function getListofTemplateFromSourceList() {
        ...
        if (SPToken) {
            var url = SPURL + templateServerRelURL + "/_api/web/lists(guid'" + TemplateLibraryDisplayName + "')/items?$select=ID,Editor/Name,Editor/Title,LinkFilename,ContentTypeId,ContentType/Id,ContentType/Name,*&$expand=File,ContentType,Editor/Id&$filter=((substringof('" + docsNodeXLSXFileExtention + "',FileLeafRef)))&$top=5000";
            $.ajax({
                beforeSend: function (request) {
                ...
                console.log('error:- ' + response.responseText);
                docsTemplateList = "<div class='displayMessage'>No Template Library found in DocsNode Admin Portal.</div>";
        ...
        try {
            ...
            getListItemsForView(selectedViewName).done(function (data) {
                ...
                if (items.length != 0) {
                    ...
                    filterstr += ") and (substringof('" + docsNodeXLSXFileExtention + "', FileLeafRef))";
                    filterstr += ")";
                    getviewdocument(filterstr).done(function (data) {
                        var pptDocument = [];
                        var response = data.d.results;
                        for (var i = 0; i < response.length; i++) {
                            if (response[i].LinkFilename.split('.').pop() == "xlsx") {
    ...
    function getviewdocument(filterstring) {
        ...
        var url = SPURL + templateServerRelURL + "/_api/web/lists(guid'" + TemplateLibraryDisplayName + "')/items?$select=ID,Editor/Name,Editor/Title,LinkFilename,ContentTypeId,ContentType/Id,ContentType/Name,*&$expand=File,ContentType,Editor/Id&$filter=";
    ...
    function getApplicationLogo() {
        try {
            var dfdMainLogo = $.Deferred();
            var responseResult = '';
            var url = SPURL + "/sites/docsnodeadmin/" + "/_api/Web/Lists/getbytitle('" + DocsNodeProductLogoListName + "')/Items?$top=1&$select=FileRef,ID,EncodedAbsUrl";
            $.ajax({
                url: url,
                method: "GET",
                headers: { "Accept": "application/json; odata=verbose", 'Authorization': 'Bearer ' + SPToken },
                success: function (responseData) {
                    if (responseData) {
                        ...
                        if (responseResult.length > 0) {
                            var results = responseResult[0];
                            // converting image URL to base64 format for client as image URL is working in office online but not working in office client.
                            imageToDataURL(results['EncodedAbsUrl'], function (dataUrl) {
                                var Logoimg = "<img src='" + dataUrl.toString() + "'/>";
                                $(".mainLOGO").html(Logoimg);
                            });
                        }
                        dfdMainLogo.resolve();
                    }
                ...
                error: function (data) {
                    console.log("getApplicationLogo: " + data);
                    dfdMainLogo.reject();
                }
            });
            dfdMainLogo.promise();
            ...
            console.log("getApplicationLogo: " + error);
        }
    }
        ...
        try {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    filterColumnList(columnFieldName);
        ...
        try {
            ...
            if (currentTemplateView == 'List') {
                for (var i = 0; i < data.length; i++) {
                    ...
                    pptString += "<i class='ms-Icon ms-Icon--ExcelLogo' title='ExcelLogo' aria-hidden='true'></i>";
    ...
    function previewToDataURL(imgURL) {
        ...
        xhr.onload = function (event) {
            if (xhr.status === 200) {
                ...
                dfdImgdef.resolve("images/icons/xls-96.png");
            }
    ...
    // convert image to base64 for client as simple image url is not working in client.
    function imageToDataURL(fileURL, callback) {
        try {
