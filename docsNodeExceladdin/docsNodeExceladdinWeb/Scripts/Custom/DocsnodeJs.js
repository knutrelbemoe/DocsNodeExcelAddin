...
BRDocsNodeJS.load = function () {
    ...
    var docsNodeXLSExtention = ".xls";
    var docsNodeXLSXExtention = ".xlsx";
    var docsNodeXLTExtention = ".xlt";
    var docsNodeXLTXExtention = ".xltx";
        ...
        try {
            ...
            if (docName == "" || docName == null || docName.match('%') || docName.match('"') ||
                ...
                getLocalForageItem("CurrentTemplateDir").done(function (values) {
                    ...
                    var docnameLen = docName.length;
                    var newFileName = docName + docsNodeXLSXExtention;
            ...
            $.ajax({
                ...
                if (!response.folderAccess) {
                    var Files = response.d.Files.results.filter(data =>
                        (data.Name.lastIndexOf(".xls") > -1 ||
                            data.Name.lastIndexOf(".xlt") > -1)
                    );
        ...
        try {
            ...
            getListItemsForView(selectedViewName).done(function (data) {
                ...
                if (items.length != 0) {
                    ...
                    filterstr += ") and (substringof('" + docsNodeXLSExtention + "', FileLeafRef))";
                    filterstr += ")";
        ...
        try {
            ...
            if (data.length > 0) {
                if (data[Count].LinkingUri != null) {
                    ...
                    imageURLs = "https://www.vippng.com/png/detail/21-210630_transparent-background-excel-icon.png";
                } else {
    ...
    function bindingWebAllTemp(imageURLs, data, i) {
        ...
        fileExt = fileExt.toLowerCase();
        if (
            data[i].Name.toLowerCase().indexOf(docsNodeXLSExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodeXLSXExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodeXLTExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodeXLTXExtention) != -1 ||
    ...
    function binding(imgdata, data, i) {
        ...
        if (data[i].Name.toLowerCase().indexOf(docsNodeXLSExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodeXLSXExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodeXLTExtention) != -1 ||
            data[i].Name.toLowerCase().indexOf(docsNodeXLTXExtention) != -1 ||
            data[i].ContentType.Name === "Folder") {
